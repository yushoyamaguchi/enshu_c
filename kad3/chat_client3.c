...
/* select() が返ってくるまでの待ち時間を指定する変数 */
  ...
  if (argc != 3) {
    fprintf(stderr, "usage: %s machine-name user-name\n", argv[0]);
    exit(1);
  }
...
if(strcmp(buf,"REQUEST ACCEPTED\n")!=0){
close(sock);
printf("error1\n");
exit(1);
}
...
write(sock,username,sizeof(username));
 memset(buf, 0, sizeof(buf));
read(sock,buf,sizeof(buf));
if(strcmp(buf,"USERNAME REGISTERED\n")!=0){
...
memset(inp, 0, sizeof(inp));
 ...
 memset(buf, 0, sizeof(buf));
read(sock,buf,sizeof(buf));
}while(strcmp(buf,"USERNAME REGISTERED\n")!=0);
